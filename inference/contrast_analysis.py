...
def build_parser():
    ...
    p.add_argument("detections",
                   help="Classification / detection JSON file or folder")
...
def analyze_contrast_loss(crop_bgr, min_delta_e=MIN_DELTA_E,
    ...
    _empty = dict(verdict="ACCESSIBLE", score=0.0, n_colors=0,
                  problematic_pairs=[], all_pairs=[], cluster_info=[])
    ...
    for i in range(len(sig)):
        for j in range(i + 1, len(sig)):
            ...
            ci_bgr = _lab_centroid_to_bgr(ci_lab).astype(np.float64)
            cj_bgr = _lab_centroid_to_bgr(cj_lab).astype(np.float64)

            gray_i = _grayscale_bt709(ci_bgr)
            gray_j = _grayscale_bt709(cj_bgr)

            lum_i = _srgb_relative_luminance(ci_bgr[2], ci_bgr[1], ci_bgr[0])
            lum_j = _srgb_relative_luminance(cj_bgr[2], cj_bgr[1], cj_bgr[0])
            cr = _wcag_contrast_ratio(lum_i, lum_j)
            ...
            pair = dict(
                ...
                grayscale_a=round(gray_i, 1),
                grayscale_b=round(gray_j, 1),
                grayscale_diff=round(abs(gray_i - gray_j), 1),
                wcag_contrast_ratio=round(cr, 2),
                is_distinct_color=bool(is_distinct),
                is_low_luminance_contrast=bool(is_low),
    ...
    if problematic_pairs:
        total_w = sum(p["pair_weight"] for p in problematic_pairs)
        max_w   = sum(p["pair_weight"] for p in all_pairs) or 1.0
        score   = min(1.0, total_w / max_w)
        ...
        for p in problematic_pairs:
            descs.append(
                ...
                f"gray {p['grayscale_a']:.0f} vs {p['grayscale_b']:.0f})")
...
def process_image(image_path, json_path, *, visualize=False,
    ...
    for fig in figures:
        ...
        if result["problematic_pairs"]:
            for pp in result["problematic_pairs"]:
                print(
                    ...
                    f"  gray={pp['grayscale_a']:.0f} vs "
                    f"{pp['grayscale_b']:.0f}"
                    f"  WCAG={pp['wcag_contrast_ratio']}:1")
        print()
        ...
        analysis_results.append(dict(
            figure_id=fid, bbox=bbox, cls=fig["cls"],
            verdict=verdict, score=result["score"],
            n_colors=result["n_colors"], reason=result["reason"],
    ...
    output_json = dict(
        ...
        thresholds=dict(
            ...
            min_data_pixels=MIN_DATA_PIXELS,
        ),
        summary=summary,
...
def process_folder(image_folder, json_folder, **kwargs):
    ...
    for jf in json_files:
        ...
        process_image(image_file, jf, **kwargs)
